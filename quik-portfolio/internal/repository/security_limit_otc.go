...
INSERT INTO
    quik.security_limits_otc (
        ...
        firm_code, firm_name, balance, acquisition_ccy, isin
    )
SELECT
...
func (r *Repository) DeleteSecurityLimitsOtcBeforeDate(ctx context.Context, date time.Time) error {
	...
	if err != nil {
		...
		r.Logger.Error("ошибка при удалении лимитов OTC по бумагам", zap.Time("LoadDate", date), zap.Error(err))
		return models.ErrSavingData
	}
	r.Logger.Debug("лимиты OTC по бумаге успешно удалены", zap.Time("LoadDate", date))
	return nil
}
...
func (r *Repository) RollSecurityLimitsOtcFromDateToDate(ctx context.Context, dateFrom time.Time, dateTo time.Time) error {
	...
	if err != nil {
		...
		if errors.As(err, &msErr) && (msErr.Number == 2627 || msErr.Number == 2601) {
			r.Logger.Warn("лимит OTC по бумаге уже существует", zap.Time("LoadDate", dateTo))
			return models.ErrConflict
		}
		r.Logger.Error("ошибка при создании лимита OTC по бумаге", zap.Time("LoadDate", dateTo), zap.Error(err))
		return models.ErrSavingData
	}
	r.Logger.Debug("лимит OTC по бумаге успешно сохранен", zap.Time("LoadDate", dateTo))
	return nil
}

func (r *Repository) SaveSecurityLimitOtc(ctx context.Context, s qmodels.SecurityLimit) error {
...
func (r *Repository) GetSecurityLimitsOtc(ctx context.Context, date time.Time) ([]qmodels.SecurityLimit, error) {
